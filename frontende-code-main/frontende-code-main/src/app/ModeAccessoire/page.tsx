...
export default function ModeAccessoirePage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products/category/id/4`);
        if (!response.ok) {
        ...
        const subcategoryNames = data.data
          .map((p: Product) => p.subcategory?.name)
          .filter((name: string | undefined): name is string => Boolean(name));
        setSubcategories(new Set<string>(subcategoryNames));
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">Mode & Accessoires</h1>
