...
const ProductCongeles = () => {
  ...
  useEffect(() => {
    const fetchProduitsFrais = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${BASE_URL}/api/products/produits-congeles`);
        const data = await response.json();
        
        if (data.success) {
          setProduitsFrais(data.data);
        } else {
          setError(data.message || "Erreur lors de la récupération des produits congeles");
        }
      } catch (error) {
        console.error("Erreur:", error);
        setError("Erreur lors de la récupération des produits congeles");
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="min-h-[200px] flex flex-col items-center justify-center bg-gray-50">
        ...
        <p className="mt-4 text-gray-600">Chargement des produits Congelés...</p>
      </div>
    );
  }
  ...
  if (produitsFrais.length === 0) {
    return (
      <div className="min-h-[200px] flex flex-col items-center justify-center bg-gray-50">
        ...
        <p className="text-gray-600">Aucun Produit Congelés disponible pour le moment.</p>
      </div>
    );
  }
...
export default ProductCongeles;
